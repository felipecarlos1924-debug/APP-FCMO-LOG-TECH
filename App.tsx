...
function useStickyState<T>(defaultValue: T, key: string): [T, React.Dispatch<React.SetStateAction<T>>] {
  ...
  // Sincronização entre abas (PC/Navegador)
  useEffect(() => {
...
export default function App() {
  ...
  // CRITICAL: Session Synchronizer Ultra-Reactive
  useEffect(() => {
    if (currentUser) {
      const updatedProfile = users.find(u => u.id === currentUser.id);
      if (updatedProfile) {
        // Comparação profunda para evitar loops, mas garantir atualização
        const hasChanged = JSON.stringify(updatedProfile) !== JSON.stringify(currentUser);
        if (hasChanged) {
          console.log("FCMO LOG TECH: Perfil atualizado detectado. Sincronizando permissões...");
          setCurrentUser({...updatedProfile});
        }
      }
    }
  }, [users, currentUser]); // Monitora mudanças na lista de usuários e no usuário atual
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-900">
      <div className={`fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm md:hidden transition-opacity duration-300 ${sidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setSidebarOpen(false)}></div>
      <div className={`fixed md:relative inset-y-0 left-0 z-[110] transform transition-transform duration-300 ease-in-out md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <Sidebar currentView={currentView} onChangeView={(v) => { setCurrentView(v); setSidebarOpen(false); }} onLogout={() => { window.localStorage.removeItem('fcmo_session_user'); setCurrentUser(null); }} currentUser={currentUser} onClose={() => setSidebarOpen(false)} />
      </div>
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-16 md:h-20 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 shadow-sm shrink-0 z-[90]">
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(true)} className="md:hidden p-2.5 text-slate-800 bg-slate-100 rounded-xl transition-all"><Menu size={24} /></button>
            <div className="hidden md:block relative w-64 lg:w-96">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
              <input type="text" placeholder="Pesquisar no sistema..." className="w-full pl-12 pr-4 py-2.5 bg-slate-100 border border-slate-200 rounded-2xl text-sm focus:ring-2 focus:ring-blue-500 outline-none font-medium text-slate-900" />
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={notificationRef}>
              <button onClick={() => setShowNotifications(!showNotifications)} className={`p-2.5 rounded-xl transition-all border ${showNotifications ? 'bg-blue-600 text-white border-blue-600' : 'text-slate-600 bg-slate-100 border-slate-200'}`}><Bell size={22} />
                {unreadCount > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-600 rounded-full border-2 border-white text-[9px] font-black text-white flex items-center justify-center shadow-md">{unreadCount}</span>}
              </button>
            </div>
          </div>
        </header>
