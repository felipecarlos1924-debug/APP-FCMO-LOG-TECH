...
import { Gauge, MapPin, Calendar, X, Save, Edit2, CheckCircle2, AlertCircle, Clock, Droplets, Building2, LayoutList, Map as MapIcon, Trash2, Camera, Eye, Truck, FileText, Zap, Thermometer, ShieldAlert, Check, UserPlus, Power, User as UserIcon } from 'lucide-react';
...
export const FleetModule: React.FC<FleetModuleProps> = ({ vehicles, checklists, fuelLogs, maintenance, branches, onAddVehicle, onUpdateVehicle, onDeleteVehicle, onAddChecklist, currentUser, users }) => {
  ...
  // Refinamento de busca: garante que se o motorista estiver em um veículo, ele o veja.
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingVehicleId) {
      // Cria uma nova referência de objeto para forçar o React a detectar a mudança
      ...
      {viewMode === 'list' ? (
        <div className="flex justify-between items-center shrink-0">
          ...
          <div className="flex gap-3">
              ...
              {canManage && (
                <button onClick={handleOpenAdd} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl transition-all active:scale-95">+ NOVO VEÍCULO</button>
              )}
          </div>
        </div>
      ) : (
        <div className="absolute top-6 right-6 z-[3000] flex gap-3 pointer-events-auto">
           <button onClick={() => setViewMode('list')} className="bg-white/95 backdrop-blur-xl border border-slate-200 p-4 rounded-3xl shadow-2xl flex items-center gap-3 text-slate-900 font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all">
              <LayoutList size={20} /> Voltar
           </button>
        </div>
      )}
                ...
                <div key={vehicle.id} className="bg-white rounded-[40px] shadow-sm border border-slate-200 overflow-hidden hover:shadow-2xl transition-all duration-500 group">
                  <div className="h-56 overflow-hidden relative">
                    ...
                    <div className="absolute top-5 right-5">
                      <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${getStatusColor(vehicle.status)} backdrop-blur-md`}>{vehicle.status}</span>
                    </div>
                  </div>
                  <div className="p-8">
                    <div className="flex justify-between items-start mb-6">
                      ...
                      {canManage && (
                         <button onClick={() => handleOpenEdit(vehicle)} className="text-slate-400 hover:text-blue-600 p-2 bg-slate-50 rounded-2xl transition-all active:scale-90"><Edit2 size={18} /></button>
                      )}
                    </div>
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-4 text-sm font-bold text-slate-700">
                         <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${vehicle.driver === 'Sem Condutor' ? 'bg-red-50 text-red-500' : 'bg-slate-100 text-slate-400'}`}><UserIcon size={20} /></div>
                         <span className={vehicle.driver === 'Sem Condutor' ? 'text-red-600' : 'text-slate-700'}>{vehicle.driver}</span>
                      </div>
                    </div>
                    <div className="mt-8 pt-6 border-t border-slate-100">
                      <button onClick={() => setViewMode('map')} className="w-full bg-slate-950 text-white py-4 rounded-[20px] font-black text-[10px] uppercase tracking-[0.2em] shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2">VER NO MAPA <MapIcon size={14} /></button>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="fixed inset-0 z-[4000] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-xl overflow-hidden animate-slide-up">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Editar Ativo</h3>
              <button onClick={() => setIsModalOpen(false)} className="w-12 h-12 flex items-center justify-center bg-white rounded-2xl text-slate-400 border border-slate-200 active:scale-90 transition-all"><X size={24} /></button>
            </div>
            <form onSubmit={handleSubmit} className="p-10 space-y-8">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Placa</label>
                  <input required type="text" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-[24px] text-slate-900 font-bold tracking-widest uppercase focus:bg-white focus:border-blue-500 outline-none" value={formData.plate || ''} onChange={e => setFormData({...formData, plate: e.target.value})} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Modelo</label>
                  <input required type="text" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-[24px] text-slate-900 font-bold focus:bg-white focus:border-blue-500 outline-none" value={formData.model || ''} onChange={e => setFormData({...formData, model: e.target.value})} />
                </div>
              </div>
              <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Motorista Responsável</label>
                  <select className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-[24px] text-slate-900 font-bold focus:bg-white focus:border-blue-500 outline-none" value={formData.driver || ''} onChange={e => setFormData({...formData, driver: e.target.value})}>
                    <option value="Sem Condutor">Sem Condutor</option>
                    {users.map(u => <option key={u.id} value={u.name}>{u.name}</option>)}
                  </select>
              </div>
              <button type="submit" className="w-full py-5 bg-slate-950 text-white rounded-[24px] font-black text-xs uppercase tracking-widest shadow-2xl transition-all active:scale-95">SALVAR ALTERAÇÕES</button>
            </form>
          </div>
        </div>
      )}
